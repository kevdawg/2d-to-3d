...
class TimingTracker:
    ...
    def step(self, step_num: int, step_name: str, show_progress: bool = True):
        ...
        # Display header
        if show_progress:
            progress_pct = int((step_num - 1) / self.total_steps * 100)
            bar_width = 40
            filled = int(bar_width * (step_num - 1) / self.total_steps)
    ...
    def print_summary(self, output_info: str = ""):
        ...
        for i, step in enumerate(self.steps):
            ...
            bottleneck_mark = " ⚠️ BOTTLENECK" if i == bottleneck_idx and percentage > 50 else ""
